        ...
        fetch(textFile)
            ...
            .then(text => {
                ...
                return {
                    ...
                    textLineLength: (lineArray && lineArray[0]) ? lineArray[0].length-1 : 0
                }
            })
            .then((fileData)=>{
                this.setState({
                    ...
                    setTimeout(() => {
                        ...
                        for (let i = 2; i < this.state.landscapeText.length; i+=2){
                            ...
                            console.log("\nline: " + i, "index: " + this.state.indexLoop[(i-1) % this.state.indexLoop.length], "indexloopindex: " + (j) % this.state.indexLoop.length);
                            if(i===2){
                                if (this.state.landscapeText[i].split('')[this.state.indexLoop[(j) % this.state.indexLoop.length]] === '#'){
                                    console.log("#");
                                    treeCount++;
                                } else {
                                    notTreeCount++;
                                    console.log(this.state.landscapeText[i].split('')[this.state.indexLoop[0]])
                                }
                            } else {
                                if (this.state.landscapeText[i].split('')[this.state.indexLoop[(j) % this.state.indexLoop.length]] === '#'){
                                    treeCount++;
                                    ...
                                    console.log(this.state.landscapeText[i].split('')[this.state.indexLoop[(j) % this.state.indexLoop.length]])
                                    notTreeCount++;
                                };
                            }

                            console.log((j) + "%" + this.state.indexLoop.length + "=>" + (j) % this.state.indexLoop.length);
                            console.log(this.state.indexLoop[(j) % this.state.indexLoop.length]);
                            j++;
                        }
                        console.log("treeCount: ", treeCount, "notTreeCount: ", notTreeCount);
                    }, 0);
                })
            })
    }
