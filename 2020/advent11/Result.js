...
async function processData(seatLinesData) {
  ...
  let checkAroundSteps = getStepsForDimensions(2, [0, 1, -1], 0).filter(
    (step) => {
      let notZero = 0;
  ...
  console.log(
    "stepping around current selected point to check neighbours by doing these steps: ",
  );
  ...
  let eqOccupiedSeats = resultObject.state
    .map((seatLine) => {
    ...
    for (let j = 0; j < currentState[i].length; j++) {
      ...
      if (currentState[i][j] !== ".") {
        checkAroundSteps.forEach((step) => {
          if (
            i + step[0] >= 0 &&
            i + step[0] < currentState.length &&
            j + step[1] >= 0 &&
            j + step[1] < currentState[i].length
          ) {
...
const getStepsForDimensions = (dimensions, stepTypes, currentIndex) => {
  var possibilities = [];
  stepTypes.forEach((stepType) => {
    if (currentIndex === dimensions - 1) {
      possibilities.push([stepType]);
    } else {
      let nextNumbers = this.getStepsForDimensions(
        dimensions,
        stepTypes,
        currentIndex + 1,
      );
      possibilities.push(
        ...nextNumbers.map((combination) => {
...
// simpler implementation of the above function
function* getCartesianProductForGridSteps(n, current = []) {
  ...
  for (const item of [0, 1, -1]) {
    yield* getAllCombinations(n, [...current, item]);
  }
}
