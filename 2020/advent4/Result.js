...
function processData(passportsRaw){
    let passports = passportsRaw.split('\n').map((line)=>{
        if(line.length === 1){
            return "[divide]";
        } else {
            return line;
    ...
    passports.forEach((passport)=>{
        processedPassports.push(passport.trim().split(' ').map((dataLine)=>{
            return {
...
function validatePassport(passport){
    ...
    passport.forEach((field)=>{
        ...
        switch (field.fieldCode) {
            case "byr":
                if( !( 1920 <= parseInt(field.fieldData) && parseInt(field.fieldData) <= 2002 )){
                    failCount++;
                ...
                break;
            case "iyr":
                if (!(2010 <= parseInt(field.fieldData) && parseInt(field.fieldData) <= 2020)) {
                    failCount++;
                ...
                break;
            case "eyr":
                if (!(2020 <= parseInt(field.fieldData) && parseInt(field.fieldData) <= 2030)) {
                    failCount++;
                ...
                break;
            case "hgt":
                let ending = field.fieldData.substring(field.fieldData.length - 2, field.fieldData.length);
                let firstPart = field.fieldData.substring(0, field.fieldData.length - 2);
                let result = false;
                if (ending === "cm" && (!isNaN(firstPart) && ((parseInt(firstPart) >= 150) && (parseInt(firstPart) <= 193)))){
                    result = true;
                }

                if (ending === "in" && (!isNaN(firstPart) && ((parseInt(firstPart) >= 59) && (parseInt(firstPart) <= 76)))) {
                    result = true;
                }
                if (!result) {
                    failCount++;
