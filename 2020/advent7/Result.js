...
function processData(rawRules) {
  ...
  rawRules.split("\n").forEach((rawRule) => {
    ...
    valueData.forEach((valueText) => {
      //going over the rules regarding one specific container's contents
      if (valueText.match(/(\d+)/)) {
        //this will return with not null, if there is a number in the valueText string
        valueArray.push({
          ...
          color: valueText
            .substring(valueText.match(/(\d+)/)["index"] + 1)
            .replace(/\.|bags|bag/g, "")
            .trim(),
  ...
  console.log(
    "Number of boxes contained by a shiny gold box: ",
    countNumberOfContainedBags("shiny gold", ruleMap, 1),
  );
}
...
function countNumberOfContainedBags(boxColor, rules) {
  ...
  rules.get(boxColor).forEach((innerBox) => {
    sum +=
      innerBox.amount +
      innerBox.amount * countNumberOfContainedBags(innerBox.color, rules);
  });
  return sum;
}
