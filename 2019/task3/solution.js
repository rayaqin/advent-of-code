    ...
    (acc, currentKey) =>
      Math.min(
        getManhattanDistance(
          0,
          0,
          intersections[currentKey].x,
          intersections[currentKey].y,
        ),
        acc,
      ),
...
const getSolutionForPart2 = (source) => {
  ...
  const closestIntersectionStepsDistance = Object.keys(intersections).reduce(
    (acc, currentKey) => {
...
const getIntersections = (mapOne, mapTwo) => {
  const intersections = {};
  Object.keys(mapOne).forEach((key) =>
    mapTwo[key] ? (intersections[key] = mapOne[key]) : null,
  );
