...
const solution = (source) => {
  ...
  for (let i = 0; i < polymer.length - 1; i++) {
    ...
    elementPairToCount.set(pair, elementPairToCount.get(pair) ? elementPairToCount.get(pair) + 1 : 1);
  }

  let newElementToPairCount = new Map();
  let counts = new Map();

  polymer.split('').forEach((a) => counts.set(a, counts.get(a) ? counts.get(a) + 1 : 1));
  ...
  console.log(sortedCountValues[sortedCountValues.length - 1] - sortedCountValues[0]);
};
