...
const solution = (source) => {
  ...
  console.log(
    algorithm,
    inputImage,
    addOuterRim(inputImage, infiniteGridState),
  );

  for (let i = 0; i < 2; i++) {
    inputImage = performOneEnhancement(
      inputImage,
      infiniteGridState,
      algorithm,
    );
...
const addOuterRim = (image, gridState) => {
  let imageWithRim = Array.from(Array(image.length + 4), () =>
    new Array(image[0].length + 4).fill(gridState),
  );
  ...
  for (let i = 0; i < imageWithRim[0].length; i++) {
    for (let j = 0; j < imageWithRim.length; j++) {
      if (
        i === 0 ||
        j == 0 ||
        i === imageWithRim[0].length - 1 ||
        j === imageWithRim.length - 1
      ) {
