...
const getSolutionForPart1 = (source) => {
  ...
  return hands.reduce(
    (totalWinnings, currentHand) =>
      totalWinnings + currentHand.bid * currentHand.rank,
    0,
  );
};
    ...
    if (jokersExist) {
      ...
      */
      if (
        cards[i] !== "J" &&
        (!keyToHighestCount ||
          charToCountMap.get(cards[i]) > charToCountMap.get(keyToHighestCount))
      ) {
  ...
  if (jokersExist) {
    ...
    */
    if (charToCountMap.has("J")) {
      ...
      charToCountMap.set(
        keyToHighestCount,
        charToCountMap.get(keyToHighestCount) + charToCountMap.get("J"),
      );
...
const isHandABetter = (handA, handB, jokersExist) => {
  ...
  for (let i = 0; i < 5; i++) {
    if (cardToValueMap[handA.cards[i]] > cardToValueMap[handB.cards[i]])
      return 1;
    if (cardToValueMap[handA.cards[i]] < cardToValueMap[handB.cards[i]])
      return -1;
  }
