...
const getSolutionForPart1 = (source) => {
  return source
    ...
    .map((card) =>
      card.drawn.reduce(
        (powerOfTwo, number) =>
          card.winning.has(number) ? powerOfTwo + 1 : powerOfTwo,
        -1,
      ),
    )
...
const getSolutionForPart2 = (source) => {
  const cards = source
    ...
    .map((card) => ({
      ...card,
      cardsWon: card.drawn.reduce(
        (cardsWon, num) => (card.winning.has(num) ? cardsWon + 1 : cardsWon),
        0,
      ),
    }));

  for (let i = 0; i < cards.length; i++) {
    getWonCardNumbers(cards[i].cardNumber, cards[i].cardsWon).forEach(
      (wonCardNumber) =>
        (cards[wonCardNumber - 1].quantity += cards[i].quantity),
    );
  }
