...
const calculateOutputValue = (entry) => {
  ...
  let lettersOfFour = entry.signalPatterns.find((a) => a.length === 4).split('');
  let lettersOfOne = entry.signalPatterns.find((a) => a.length === 2).split('');
  let lettersOfSeven = entry.signalPatterns.find((a) => a.length === 3).split('');
  let lettersOfEight = entry.signalPatterns.find((a) => a.length === 7).split('');
  ...
  for (let letter of rightToWrongLetter.keys()) {
    ...
    } else if (lettersOfSeven.includes(letter) && commonAmongFives.includes(letter)) {
      rightToWrongLetter.set('a', letter);
    } else if (lettersOfFour.includes(letter) && !commonAmongFives.includes(letter) && !lettersOfOne.includes(letter)) {
      rightToWrongLetter.set('b', letter);
    } else if (!lettersOfFour.includes(letter) && !commonAmongFives.includes(letter) && lettersOfEight.includes(letter)) {
      rightToWrongLetter.set('e', letter);
    } else if (lettersOfOne.includes(letter) && findLetterFrequencyInPatterns(letter, patternsOfSix) === 2) {
      rightToWrongLetter.set('c', letter);
    } else if (commonAmongFives.includes(letter) && !lettersOfFour.includes(letter) && !lettersOfSeven.includes(letter)) {
